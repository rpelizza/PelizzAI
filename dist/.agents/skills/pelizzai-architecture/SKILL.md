...
reclassify to `write-local`, and go through `pelizzai-isolate` **before** the first write.
...
- an active bug (`pelizzai-diagnose`);
...
Use `pelizzai-module-design` as vocabulary/lens, not as a second head skill. Useful reasoning:
