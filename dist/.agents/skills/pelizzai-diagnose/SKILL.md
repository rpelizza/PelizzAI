...
through `pelizzai-isolate` **before** the edit; any eventual deploy also goes through the
...
`pelizzai-discovery` (feature track). Without that discussion there is no fix #4.
...
`pelizzai-isolate` discovers the base and proposes the name WITHOUT a stop of its own (a
...
- UI: pelizzai-interface is a mandatory overlay; behavior when applicable + visual verification.
...
If no adequate seam exists for an important regression, record the architectural finding and route it to `pelizzai-architecture`; do not write a tautological test at the wrong seam.
...
`pelizzai-architecture` with the vocabulary of `pelizzai-module-design`.
