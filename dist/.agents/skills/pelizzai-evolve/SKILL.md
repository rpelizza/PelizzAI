...
schema (`pelizzai-onboard` seeds both at bootstrap; both are **versioned**, like the rest of
...
| --- | --- | --- | --- |
| **Defect** | an observed failure | fix it, inside the guardrails | none, once confirmed (`pelizzai-diagnose` proves it is real and repeatable) |
...
- **Domain skills** stay with `pelizzai-skill-lab` and its ledger
...
standard's readers; `pelizzai-plan`, `pelizzai-execute`, `pelizzai-quick-fix`, and
`pelizzai-diagnose` — the readers of the Active rules, one per track, so no route reaches a decision
without them; `pelizzai-onboard` — seeds both artifacts at bootstrap; `pelizzai-skill-lab` — the
