---
name: pelizzai-discovery
description: "Use before implementing a greenfield product or any change with open trade-offs in requirements, architecture, UX, data, or risk. Turns intent into a design the user decides and ratifies."
---
...
For UI, apply the `pelizzai-interface` overlay at design time: real flow, states, content, existing system, accessibility, and visual direction. This does not authorize implementing before approval.

For modules, use `pelizzai-module-design` only when boundaries/seams are still a decision. For the domain, use `pelizzai-domain-modeling` when the vocabulary/ADR actually changes — not merely to read the glossary.
...
- Normal flow: hand the approved spec to `pelizzai-plan`.
- New project — design-edge closeout checklist, each step mandatory before leaving design:
  1. Design approved → invoke `pelizzai-onboard` (proactive domain skills gate): propose the set for the decided stack, with context7; the decision is the user's.
  2. Create only the ratified ones and record them in the catalog/ledger.
  3. Proceed to `pelizzai-plan` when the original request includes building the product; stop after design/bootstrap only when that was the requested scope.

## Prototypes

Use `pelizzai-experiment` only when a state/logic/visual-form question cannot be answered economically by analysis. The prototype:
...
[ ] new project: the stack's domain skills proposal was presented at the edge (the proactive gate
    of `pelizzai-onboard`) before proceeding to the plan, and the user's decision is recorded;
