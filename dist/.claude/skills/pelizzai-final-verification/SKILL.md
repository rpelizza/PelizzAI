---
name: pelizzai-final-verification
...
using the PelizzAI Final Verification skill to confirm with evidence before declaring
...
`pelizzai-finish` closes out in `phase: delivered` (sealed content + destination executed), never in `done`;
...
At pelizzai-finish entry in a consumer:

- `git rev-parse HEAD == validated-head`;
- the working tree is clean — the pending seal update lives in `pelizzai/data/state.md`, the
  local per-dev cursor git ignores (issue #43), invisible to porcelain;
- no evidence predates the last fix/overlay.

`pelizzai-finish` creates exactly one metadata-only closure commit. Before push/PR, it proves that
`validated-head..closure-head` contains only harness metadata: the
`pelizzai/data/history/<YYYY-MM-DD>-<slug>-<sha7>.md` file generated by the seal migration. That commit
does not change the validated content.

In source mode, pelizzai-finish receives `validated-head` from the execution record, requires an equal
