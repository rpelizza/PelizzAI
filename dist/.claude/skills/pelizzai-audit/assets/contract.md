...
- structural decisions (base/branch, isolation, mode with `team` visible, commits, destination) are recommended and ratified — one at a time in tracks with a plan; in tweak/bug, in a compact one-line confirm with all of them visible and named; in the bootstrap, in `pelizzai-audit`'s own compact confirm — never as a silent default. The review is NOT among them: its form follows the contract (two lenses in two dispatches wherever a ratified plan/spec exists; the quality lens alone where the flow produced its own artifact), so there is no question to ask;
