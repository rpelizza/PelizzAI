...
## Bootstrap mode (called by `pelizzai-onboard`)

Triggered in `bootstrap-write`, after `pelizzai-onboard` has mapped the context and created the task branch. Scan-only does not call this mode. **Consumer only:** in the PelizzAI source repo there is no consumer bootstrap and no `pelizzai/` runtime — steps 4–5 below write nothing under `pelizzai/`; the equivalent record lives in the native execution record.

```text
1. Receive the evidenced inventory from pelizzai-onboard (stacks, frameworks, modules, conventions,
...
- **Version-driven (refresh — UPDATES):** the stack changed major version or gained a significant dependency → re-read the current version's docs (context7) and **update** the affected existing skill. Drift is detected by comparing the current manifests against the **Stack baseline** in `pelizzai/profile.md` (written by `pelizzai-onboard` at bootstrap).
...
In a CONSUMER, editing the BODY of a domain skill opens the review of its catalog entry in the
same step: the
...
enters the catalog; this closes the reverse loop. In the source repo neither catalog nor ledger
exists: record the same fact in the native execution record and create no `pelizzai/` file.
...
- Correcting a skill's body and leaving its catalog entry announcing the old assertions
  (consumer; in source mode the native execution record carries that fact).
