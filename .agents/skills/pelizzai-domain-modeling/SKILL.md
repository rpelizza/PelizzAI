...
`pelizzai-ideia-generation` uses this overlay only when the model changes; `pelizzai-writing-plans`
...
`pelizzai-ideia-generation` when saving the spec of a ratified design (auto + one-line announcement);
