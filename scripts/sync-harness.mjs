 ...
 * it was there first.
 */
// Legacy signature: a file generated by pre-block versions of this script STARTS with the complete
// two-line generated header. BOTH lines are required verbatim at the very top: authored text that
// merely mentions or partially quotes the header stays authored.
const LEGACY_HEADER =
  /^\s*<!-- GENERATED by scripts\/sync-harness\.mjs from CLAUDE\.md — do NOT edit by hand\. -->\r?\n<!-- To change the guidelines, edit CLAUDE\.md and run your platform's sync-harness\. -->/;

/** The text of a generated file that lives OUTSIDE its managed block — what the project authored. */
function unmanagedText(text) {
  const start = text.indexOf(MANAGED_BEGIN);
  const end = text.indexOf(MANAGED_END);
...
/**
 * True when the unmanaged part of a generated file still carries a copy of the doctrine: the legacy
 * whole-file output that a later block was written UNDER instead of over (issue #84 — the source
 * repo's AGENTS.md carried the guidelines twice for four days, and `--check` compared only the
 * block). Codex and Gemini load these files whole, so a duplicate doubles the entry cost per turn.
 */
function hasDuplicateDoctrine(text) {
  const outside = unmanagedText(text);
  return LEGACY_HEADER.test(outside.trim()) || outside.includes('## Behavioral guidelines');
}
...
function writeManaged(path, content) {
  ...
  if (start >= 0 && end > start) {
    // A legacy whole-file output sitting ABOVE the block is generated text, not the project's: it
    // is the stale copy issue #84 found, and it goes. Genuine authored text above the block stays.
    const head = LEGACY_HEADER.test(text.slice(0, start).trim()) ? '' : text.slice(0, start);
    const tail = text.slice(end + MANAGED_END.length).replace(/^\n/, '');
    return writeTextAtomic(path, `${head}${block}${tail}`);
  }
  const existing = text.trim();
  // Legacy migration: a wholly generated file (no markers, legacy header on top) is replaced.
  // Appending would leave TWO copies of the doctrine — the stale one on top, with a skill list
  // that no longer exists. Only genuine project-authored text earns the append.
  if (LEGACY_HEADER.test(existing)) {
...
function check(sourceMode) {
  ...
  for (const generated of [agentsMd, geminiMd]) {
    if (existsSync(generated) && hasDuplicateDoctrine(readText(generated))) {
      console.error(
        `FAIL: ${basename(generated)} carries the doctrine outside its managed block (a stale legacy copy). Run the sync to rewrite it.`,
      );
      problems += 1;
    }
  }
