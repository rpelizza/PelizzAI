...
Wrappers equivalentes: scripts/sync-harness.ps1 e scripts/sync-harness.sh.`);
  ...
  if (
    ...
    throw new Error('--build-dist não combina com outras flags.');
  }
  return options;
}
...
function buildDist({ quiet = false } = {}) {
  if (!existsSync(sourceSentinel)) {
    throw new Error('--build-dist só roda no repo-fonte (sentinela ausente).');
  }
  ...
  if (!quiet) {
    console.log(
      `dist/ regenerada (${core.length} skills core; sem sentinela): copie o conteúdo dela para a raiz do seu projeto.`,
    );
  }
}
...
function check(sourceMode) {
  ...
  if (sourceMode) {
    ...
    if (!existsSync(join(distDir, '.cursor', 'rules', 'pelizzai.mdc'))) {
      distProblems.push('adaptador Cursor ausente');
    }
    if (existsSync(join(distDir, 'scripts', 'pelizzai-source-repo.txt'))) {
      distProblems.push('sentinela de source mode presente');
    }
    if (existsSync(join(distDir, 'scripts', 'test-harness-contracts.ps1'))) {
      distProblems.push('suíte de contratos presente');
    }
    const distSkillsDiff = treeDiffCount(srcSkills, join(distDir, '.claude', 'skills'));
    if (distSkillsDiff) distProblems.push(`skills fora de sincronia (${distSkillsDiff} arquivo(s))`);
    if (distProblems.length) {
      console.error(`FAIL: dist/ inválida: ${distProblems.join('; ')}. Rode o sync (ou --build-dist).`);
      problems += 1;
    }
  }
...
function generate(updateManifest) {
  ...
  if (existsSync(sourceSentinel)) {
    console.log('Regenerando dist/ (instalação por cópia)…');
