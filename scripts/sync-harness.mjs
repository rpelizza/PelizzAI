...
function writeManaged(path, content) {
  ...
  // Legacy migration: a file generated by pre-block versions of this script STARTS with the
  // complete two-line generated header and has no markers. Appending would leave TWO copies of
  // the doctrine — the stale one on top, with a skill list that no longer exists. A wholly
  // generated file is replaced; only genuine project-authored text earns the append. BOTH header
  // lines are required verbatim at the very top: authored text that merely mentions or partially
  // quotes the header stays authored and gets the append.
  const legacyHeader =
