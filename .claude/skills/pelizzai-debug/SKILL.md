...
If no adequate seam exists for an important regression, record the architectural finding and route it to `pelizzai-architecture-refinement`; do not write a tautological test at the wrong seam.
...
[ ] Consumer only: incident entry (status candidate) appended to pelizzai/data/learnings.md in
    ...
    already in the corpus — pelizzai/data/learnings.md plus every existing
    pelizzai/data/history/learnings-<YYYY>.md, enumerated, pointer or no pointer — is a
    RECURRENCE; flag it for promotion at closeout.
```
...
`pelizzai-architecture-refinement` with the vocabulary of `pelizzai-codebase-architecture`.
