...
- Threshold crossed → propose ONCE:
  "We have accumulated <count> commits / <days> days since the last domain-skill review.
   May I run maintenance (pelizzai-create-skill) now? Proceed now or leave it for later?"
...
**Who installs it (opt-in):** at bootstrap, `pelizzai-create-skill` **proposes** the installation; if the user accepts, it **merges** the entry into `.claude/settings.json`, preserving existing hooks and permissions (merge, **never** overwrite the file). In Claude Code, the `update-config` skill can perform this edit. Also add `pelizzai/data/.cadence-state.json` to `.gitignore` — it is mutable state (changes on every interaction) and must not be versioned.
