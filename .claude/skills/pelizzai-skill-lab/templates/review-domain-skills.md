...
> Read by `pelizzai-skill-lab` (cadence) and by `pelizzai-onboard` (bootstrap).
> - Seeded by: `pelizzai-skill-lab` at bootstrap (orchestrated by `pelizzai-onboard`), with the **bootstrap date** (the bootstrap is the 1st review; seeding with the 1st commit would fire a spurious nudge on the first task).
> - Updated by: `pelizzai-skill-lab` on every domain-skill creation/refresh and after every review.
