...
description: Use essa skill sempre que for criar, editar, otimizar ou MANTER uma skill — as do harness e, principalmente, as skills de DOMÍNIO de um projeto. É o motor de autoria do PelizzAI. Acione-a (a) no bootstrap, chamada pela `pelizzai-audit`, para gerar o máximo de skills de domínio a partir do repo-scan, fundamentadas no MCP `context7` e nas regras de skills da Anthropic; e (b) na manutenção — quando a stack muda de versão, quando padrões se repetem no histórico do git, ou quando a cadência (≥10 commits, >10 dias, ou repo-scan a cada 10 dias) indica revisão. NUNCA sobrescreve uma skill às cegas: mostra o diff, preserva customizações e pede confirmação por skill. Acione também quando o usuário disser "criar skill", "transforme isso numa skill", "otimizar a descrição", "atualizar/manter as skills de domínio".
---
...
- **Estilo:** imperativo nas instruções; explique o porquê; comece com rascunho e melhore com um olhar renovado.
...
3. Use um TIME (pelizzai-team) ou subagentes (pelizzai-subagents) para redigir em PARALELO —
...
5. Semeie o ledger com a data do 1º commit do repo (ver Templates) e escreva o catálogo
   `pelizzai/domain-skills.md` — sua existência marca o bootstrap como concluído.
6. Apresente ao usuário a lista de skills criadas (catálogo) para revisão. Nada é definitivo sem o aval dele.
```
...
- **`pelizzai/data/review-domain-skills.md`** — ledger: por skill, data de criação, última atualização, último commit/ref revisado e o eixo da mudança; + `last-review` e `last-full-scan` globais. Template: [templates/review-domain-skills.md](templates/review-domain-skills.md).

Semeie o ledger com a **data do 1º commit** (`git log --reverse --format=%cd --date=short | head -1`) para o contador funcionar desde a primeira tarefa.
...
- `pelizzai-team` / `pelizzai-subagents` — redigir muitas skills candidatas em paralelo.
